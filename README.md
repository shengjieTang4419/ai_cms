...
**从基础对话到智能 Agent，从单体架构到微服务拆分，再到云原生部署**

[功能特性](#-核心功能) • [快速开始](#-快速开始) • [技术架构](#-技术架构) • [微服务部署](#-微服务部署) • [API 文档](#-api-测试) • [更新日志](#版本历史)
...
这是一个基于 **Spring AI** 和 **Spring AI Alibaba** 构建的**智能个性化对话系统**
，集成了大语言模型（LLM）、向量数据库、用户画像、个性推荐等核心技术，实现了完整的生产级 AI 应用。

### 📊 项目统计

| 指标         | 数值        | 说明                                     |
|------------|-----------|----------------------------------------|
| **总功能数**   | 25 项      | V1.0: 7项，V2.0: +14项，V3.0: +4项          |
| **代码行数**   | ~20,000 行 | Java + Vue 3                           |
| **微服务数量**  | 5 个       | auth/gateway/system/membership/ai-chat |
| **支持模型**   | 3 家       | DashScope / OpenAI / DeepSeek          |
| **MCP 工具** | 6 个       | 天气/位置/路线/数据库/推荐/搜索                     |
| **多模态**    | 3 种       | 文本 / 图像(OCR) / 语音(ASR)                 |
| **开发周期**   | 3 个月      | 2024.11 - 2025.01                      |
...
- 打造生产级 AI 对话系统
- 实践微服务架构设计与拆分
- 掌握 Spring Cloud 生态组件
- 实现云原生部署方案

### ✨ V3.0 核心升级（2025年12月-2025年1月）

#### 🏗️ 微服务架构层面（4项）

- ☁️ **完整微服务拆分**：拆分为 cloud-auth、cloud-gateway、cloud-system、cloud-membership、cloud-ai-chat 五大独立服务
- 🔗 **服务注册发现**：集成 Nacos 注册中心，实现服务自动注册与发现
- ⚙️ **配置中心统一**：Nacos 配置中心，统一管理各微服务配置
- 🛡️ **API 网关统一**：Spring Cloud Gateway 统一入口，路由转发 + 负载均衡 + 限流熔断

#### 🔐 安全认证层面（2项）

- 🎫 **统一认证中心**：cloud-auth 提供统一 JWT 认证服务
- 🔄 **Token 无感刷新**：实现 Token 自动刷新机制，提升用户体验

#### 📦 部署层面（2项）

- 🐳 **Docker 容器化**：各微服务 Docker 镜像构建
- 🚢 **Docker Compose**：一键启动所有微服务及依赖中间件

**总计新增功能：8 项** | **累计功能：29 项**
...
├── cloud-auth/                     # 认证服务中心 🆔
│   └── cloud-auth-biz/             # 认证业务实现
├── cloud-gateway/                  # API 网关服务 🚪
│   └── cloud-gateway-biz/          # 网关路由配置
├── cloud-system/                   # 系统管理服务 ⚙️
│   ├── cloud-system-api/           # 系统服务接口
│   └── cloud-system-biz/           # 系统服务实现
├── cloud-membership/               # 会员标签服务 👤
│   ├── cloud-membership-api/       # 会员服务接口
│   └── cloud-membership-biz/       # 会员服务实现
├── cloud-ai-chat/                  # AI 聊天核心服务 🤖
│   ├── cloud-ai-chat-api/          # AI聊天服务接口
│   └── cloud-ai-chat-biz/          # AI聊天服务实现
│       ├── provider/               # SPI模型提供者
│       ├── service/                # 业务服务层
│       ├── mcp/                    # MCP工具（天气、推荐等）
│       ├── controller/             # REST API
...
本项目采用**完整的微服务架构**，将系统按业务域拆分为 5 个独立服务：

```mermaid
graph TB
    Client[客户端] --> Gateway[cloud-gateway:8080]
...
| 服务                   | 端口   | 职责     | 核心功能                 |
|----------------------|------|--------|----------------------|
| **cloud-gateway**    | 8080 | API 网关 | 路由转发、负载均衡、限流熔断、统一鉴权  |
| **cloud-auth**       | 8081 | 认证中心   | 用户认证、JWT 生成、Token 刷新 |
| **cloud-system**     | 8082 | 系统管理   | 用户管理、角色权限、系统配置       |
| **cloud-membership** | 8083 | 会员服务   | 用户画像、标签管理、个性推荐       |
| **cloud-ai-chat**    | 8084 | AI 聊天  | 对话服务、RAG 问答、多模态处理    |

### 服务通信

- **同步调用**：Feign + OpenFeign 进行服务间 HTTP 调用
- **注册发现**：Nacos 作为注册中心，服务自动注册与发现
- **配置管理**：Nacos 配置中心，统一管理各服务配置
...
- 前端若检测到“怎么去/路线”等关键词，会请求浏览器定位并将经纬度透传到 `streamChat`，Prompt 会附带 `[我的当前位置坐标：lng,lat]`
  ，模型可直接调用 MCP 工具生成路线方案。
